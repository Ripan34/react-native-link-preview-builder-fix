...
export const getPreviewData = async (text, requestTimeout = 5000) => {
    ...
    try {
        ...
        let openGraphUrl='';
        meta.forEach(ele => {
            metaStr += ele[0].toString();
        })
        let arr = metaStr.split("<meta ");
        arr.forEach((ele)=>{
            if((ele.includes('property="og:title"') || ele.includes('name="og:title"')) && openGraphTitle==''){
                let temp = ele.split('content=\"')[1]
                openGraphTitle = temp.split(`"`)[0];
            }
            else if((ele.includes('property="og:description"') || ele.includes('name="og:description"')
            ||ele.includes('property="description"') || ele.includes('name="description"')) && openGraphDescription==''){
                let temp = ele.split('content=\"')[1]
                openGraphDescription = temp.split(`"`)[0];
            }
            else if((ele.includes('property="og:image"') || ele.includes('name="og:image"')) && openGraphUrl==''){
                let temp = ele.split('content=\"')[1]
                openGraphUrl = temp.split(`"`)[0];
            }
        })
        if(openGraphTitle != ''){
            return {
                description: openGraphDescription,
                image: await getPreviewDataImage(openGraphUrl),
                link: openGraphUrl,
